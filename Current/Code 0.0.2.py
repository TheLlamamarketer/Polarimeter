...
def calibrated_spline():
    ...
    x_range = np.linspace(xdata.min(), xdata.max(), 10000)  # Adjust the number of points
    ...
    for i in range(1, len(derivative_values) - 1):
        if derivative_values[i - 1] > 0 and derivative_values[i] < 0:
            local_extrema.append((x_range[i], 'Maximum'))
        elif derivative_values[i - 1] < 0 and derivative_values[i] > 0:
            local_extrema.append((x_range[i], 'Minimum'))
    ...
    ax1.plot(xdata, spline_prime(xdata), 'r-', label='Derivative of Spline fit')  
    ax1.legend()
